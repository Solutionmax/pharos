...
@include('partials.pagehead', [
  'title' => 'Settings',
  'sub' => 'What the status page shows, and how it looks by default',
])
  ...
  <div>
    <form method="POST" action="{{ route('admin.settings.update') }}" id="settings-form">
      ...
      <div class="panel">
        <div class="panel-hd">
          <h3>Services</h3>
          <span class="hint"><a href="{{ route('admin.groups', ['from' => 'settings']) }}">Rename or add</a></span>
        </div>
        <div class="panel-bd">
          ...
          @empty
            <div class="callout">
              No services yet. <a href="{{ route('admin.groups', ['from' => 'settings']) }}">Add one</a> and it appears here
              as its own switch.
            </div>
          @endforelse
        </div>
...
(function () {
  ...
  var base = @json(route('admin.settings.preview'), JSON_UNESCAPED_SLASHES);
  var timer = null;
