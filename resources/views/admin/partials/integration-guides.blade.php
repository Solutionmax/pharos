...
<section class="panel" id="incoming-integrations" data-integration-base="{{ url('/api/v1') }}">
  ...
  <div class="panel-bd">
    ...
    <div class="field integration-component">
      ...
      @if ($manualComponents->isEmpty())<a class="integration-link" href="{{ route('admin.components.create') }}">Add a component</a>@endif
    </div>
    ...
    <details class="integration-guide" id="guide-n8n" open>
      ...
      <div class="integration-guide-body">
        ...
        <pre id="n8n-component-url" data-component-url="components">{{ url('/api/v1/components/'.$guideComponent) }}</pre>
        ...
        <details class="integration-example"><summary>Need an incident and customer updates instead?</summary>
          <p>POST this body to <code>{{ url('/api/v1/incidents') }}</code> with the same authorization header. Store the returned <code>data.id</code>. For later updates, POST a status and message to <code>/api/v1/incidents/INCIDENT_ID/updates</code>; use <code>resolved</code> to close it. Each create request opens a new incident, so do not create one on every poll.</p>
    ...
    <details class="integration-guide" id="guide-kuma">
      ...
      <div class="integration-guide-body">
        ...
        <pre id="kuma-url" data-component-url="kuma/components">{{ url('/api/v1/kuma/components/'.$guideComponent) }}</pre>
    ...
    <details class="integration-guide" id="guide-scripts">
      ...
      <div class="integration-guide-body">
        ...
        <pre id="incident-curl">curl -X POST '{{ url('/api/v1/incidents') }}' \
    ...
    <details class="integration-guide" id="guide-heartbeats">
      ...
      <div class="integration-guide-body">
        ...
        @forelse ($heartbeats as $component)
          ...
          <pre id="heartbeat-{{ $component->id }}">{{ url('/api/v1/heartbeat/'.$component->check->target) }}</pre>
        ...
        @empty
          ...
          <a class="integration-link" href="{{ route('admin.components.create') }}">Add a heartbeat component</a>
        @endforelse
      </div>
    </details>
  </div>
