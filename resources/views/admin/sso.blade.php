...
@include('partials.pagehead', [
  ...
  'sub' => $sso->enabled() ? 'On · '.$sso->providerName() : 'Off',
])

<div class="panel">
  <div class="panel-hd"><h3>OpenID Connect</h3><span class="hint">Authentik, Keycloak, Entra, Google, Okta</span></div>
  <div class="panel-bd">
    <div class="callout">
    ...
    <form method="POST" action="{{ route('admin.sso.update') }}" style="display:flex;flex-direction:column;gap:16px">
      ...
      <div class="actions">
        ...
        <a class="btn ghost" href="{{ route('admin.integrations') }}">Cancel</a>
      </div>
    </form>
  </div>
</div>
...
<div class="panel">
  <div class="panel-hd"><h3>What still applies</h3></div>
  <div class="panel-bd">
    <p class="sub" style="font-size:13.5px;color:var(--ink-2)">
      Anyone who switched two-factor on keeps it, whatever door they came through. If your provider
      already enforces MFA and you would rather not be asked twice, switch your own two-factor off
      on your profile — that stays your decision, not the login screen's.
    </p>
  </div>
</div>
@endsection
