...
- [ ] Write tests for default migration, separate settings/subscribers, context restoration on exception, ownership filtering and assignment.
```php
...
```
- [ ] Run `vendor/bin/phpunit tests/Feature/PageOwnershipTest.php`, confirm missing behavior.
- [ ] Implement migration/model/context contract; use database backfill without Eloquent boot side effects.
- [ ] Run new tests and full existing suite, fix compatibility regressions; commit only task files.
...
- [ ] Tests request A with B ids (read/write/preview/export), list routes, multi-tab links, legacy routes, missing assignment and revoked token owner.
```php
...
```
- [ ] Run targeted test, implement routes/context middleware and scoped exists/in rules; reject cross-page component/group attachments.
- [ ] Bind token rights to current page and current owner rights, preserve legacy system token default-page scope.
- [ ] Verify default routes and all existing request tests; commit task files.
...
- [ ] Test mixed page batch, address on two pages, private incident after queue, archived page, failed SMTP never falls back.
```php
$this->assertSame([$subscriberA->id], SubscriberNotification::withoutGlobalScope('status_page')->pluck('subscriber_id')->all());
```
- [ ] Run failing tests; implement explicit contexts and finally restoration for every background path.
- [ ] Namespace uploaded brand assets by page; use canonical saved page URLs in template links and signed subscription routes.
- [ ] Test two messages in same process with different branding/from/mailer and existing subscriptions/notifier/check/webhook tests; commit task files.
...
- [ ] Test free second-page denial, paid creation, limit and invalid claims, archive/default restrictions and editing existing page after expiration.
```php
$this->actingAs($admin)->post('/admin/pages', ['name'=>'B','slug'=>'b'])->assertForbidden();
$this->assertSame(1, StatusPage::count());
```
- [ ] Implement signed `multi_pages` feature and optional validated `limits.status_pages`. Transaction locks default page while count/check/insert/reactivation happen.
- [ ] Add page list/create/edit/archive and assignments UI; domain is unique normalized host, optional and only installation admin can activate after DNS/TLS confirmation.
- [ ] Preserve branding on existing pages when applicable, preserve current perpetual Brand Pack semantics; expired license prevents new pages only.
- [ ] Wire selector with explicit links and empty-state for unassigned users; verify admin role boundaries.
- [ ] Run task tests and lint, commit.
...
- [ ] Review whole branch for unscoped queries, page-agnostic URL helpers, raw DB validations, secrets in audit, stale mailer config and inherited relationship leaks.
- [ ] Run `vendor/bin/phpunit`, `vendor/bin/pint --test`, `vendor/bin/phpstan analyse --memory-limit=1G`, `git diff --check`.
- [ ] Rehearse upgrade from populated 0.6.0 SQLite and MySQL with fake transports; verify ids/history, old links/tokens and page-unique email behavior.
- [ ] Serve isolated local preview with two synthetic brands; browser-check creation/navigation/branding/service separation at desktop and mobile sizes.
